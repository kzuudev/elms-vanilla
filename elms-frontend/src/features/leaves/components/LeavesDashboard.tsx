...
export default function LeavesDashboard({ role }: { role: string }) {
  ...
  const [reviewerLeaveRequests, setReviewerLeaveRequests] = useState<
    ReviewerLeaveData[] | null
  >(null);
  const [leaveRequestDetails, setLeaveRequestDetails] = useState<LeaveRequest>(
    {} as LeaveRequest,
  );
      ...
      if (axios.isAxiosError(e)) {
        setError(
          e.response?.data?.message ??
            "Failed to fetch personal leave requests",
        );
        return;
      }
    }
  ...
  const fetchLeaveRequestDetails = async (id: number) => {
    try {
      ...
      if (axios.isAxiosError(e)) {
        setError(
          e.response?.data?.message ?? "Failed to fetch leave request details",
        );
        return;
      }
