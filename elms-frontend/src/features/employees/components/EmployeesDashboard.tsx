...
import type { Department, DepartmentOptions } from "@/types/department.ts";
...
export default function EmployeesDashboard({ role }: { role: string }) {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      const message = getApiErrorMessage(error, "Failed to load departments");
      if(!message) {
        return;
      }
      setError(message);
         ...
         <div className="flex justify-between">
                  ...
                  {isAdmin ? (
                    <>
                      ...
                      <p className="text-gray-500 text-xs">
                        Manage your department employees
                      </p>
                    </>
                  ) : isSuperAdmin ? (
                    <>
                ...
                {(isAdmin || isSuperAdmin) && (
                  <Dialog open={isFormOpen} onOpenChange={setIsOpenForm}>
                    ...
                    <DialogContent className="w-full sm:max-w-[425px] py-6 px-2 bg-white border-0 outline-none shadow-xl">
                      <Register closeDialog={() => setIsOpenForm(false)} departments={departmentOptions} />
                    </DialogContent>
                  </Dialog>
                )}
              </div>
          ...
          <div>
            {employees?.some((employee) =>
              ...
              <EmployeesListTable
                employees={employees}
                onUserMutated={() => fetchEmployees(filters)}
              />
            ) : (
              <div className="text-center text-gray-500">
                <p>
                  No employees found for{" "}
                  {`"${filters.searchQuery + " " + filters.statusFilter + " " + filters.departmentFilter + " " + filters.roleFilter}"`}
                </p>
              </div>
            )}
          </div>
