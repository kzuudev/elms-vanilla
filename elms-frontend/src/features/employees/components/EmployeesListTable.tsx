...
import { type Manager, type Admin } from "@/types/employees.ts";
...
export default function EmployeesListTable({
  ...
  const handleEditEmployeeForm = async (id: number) => {
    ...
    if (data) {
      form.reset({
        ...
        assigned_to:
          data.assigned_to?.id != null ? String(data.assigned_to.id) : "",
  ...
  const fetchUpdateEmployee = async (
    id: number,
    data: EditEmployeeDetailsFormData,
  ) => {
    form.setError("root", {
      type: "server",
  ...
  const fetchManagers = async () => {
    try {
      ...
      setManagers(
        (response.data.data.managers ?? []).map((manager: Manager) => ({
          id: manager.id,
    ...
    try {
      ...
      if (axios.isAxiosError(e)) {
        setError(
          e.response?.data?.error ||
            e.response?.data?.message ||
            "Failed to load admins from the server",
        );
      } else {
    ...
    if (employeeDetails) {
      form.reset({
        ...
        hired_date: employeeDetails.hired_date
          ? employeeDetails.hired_date.split(" ")[0]
          : "",
        salary: employeeDetails.salary ? String(employeeDetails.salary) : "",
        assigned_to:
          employeeDetails.assigned_to?.id != null
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="w-full flex flex-col gap-4">
              <div className="w-full flex flex-col gap-2">
                ...
                employeeDetails?.leave_balance.length > 0 ? (
                ...
                <div className="flex justify-between">
                  ...
                  {employeeDetails?.hired_date
                    ? format(
                        new Date(employeeDetails.hired_date),
                        "MMMM dd, yyyy",
                      )
                    : "N/A"}
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  {employeeDetails?.is_active === 1 ? (
                    <span className="bg-green-50 text-green-700 border border-green-200/60 px-2 py-1 rounded-md">
                      active
                    </span>
                  ) : employeeDetails?.is_active === 0 ? (
                    <span className="bg-red-50 text-red-700 border border-red-200/60 px-2 py-1 rounded-md">
                      inactive
                    </span>
                  ) : null}
                </div>
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState }) => (
                        <Field
                          ...
                          className="w-1/2"
                        >
                          <FieldLabel htmlFor="is_active" className="m-0">
                            Account Status
                          </FieldLabel>
                          <Select
                            {...field}
              ...
              <TableRow key={employee.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    ...
                    {/* Manager Actions */}
                    {isManager && (
                      <Button variant="outline" className="p-2 mr-1">
                        <Calendar />
                      </Button>
                    )}
