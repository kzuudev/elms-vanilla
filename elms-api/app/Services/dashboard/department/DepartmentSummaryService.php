...
class DepartmentSummaryService {
    ...
    public function getSmallestDepartment() {
        $this->checkUserRole();

        $smallest_department = $this->db->query("
            SELECT d.id, d.name AS department_name, COUNT(u.id) AS total_employees
        ", [
            'current_user_id' => $this->user['id']
        ])->find();
    ...
    public function getAverageEmployeesPerDepartment() {
        $this->checkUserRole();

        $average_employees_per_department = $this->db->query("
            SELECT AVG(total_employees) AS average_employees_per_department
        ", [
            'current_user_id' => $this->user['id']
        ])->find();
    }
